...
module Validator::Rules
  class Max < Size
    ...
    def error_message(values : Hash(String, String)) : String
      value = @value
      m = case value
          when value.nil?          then "be at most #{@size.to_s}"
          when value.is_a?(Int)    then "be at most #{@size.to_s}"
          when value.is_a?(Array)  then "contain at most #{@size.to_s} items"
          when value.is_a?(Hash)   then "contain at most #{@size.to_s} items"
          when value.is_a?(String) then "be at most #{@size.to_s} characters long"
          else                          "be at most #{@size.to_s}"
          end
